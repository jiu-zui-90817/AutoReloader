...
def replace_ini_section(filepath, section, block_text):
    if not os.path.exists(filepath):
        with open(filepath, "w", encoding="ansi") as f: f.write("; Auto-generated by Tactical Console\n")
    ...
    if fp:
        ...
        if filename != "hotfix.ini":
            messagebox.showwarning(
                "⚠️ 文件选择受限", 
                "为了保护您的源码不被覆盖，战术工坊只能操作专属的临时工程草稿文件！\n\n"
                "✅ 仅允许的文件名：hotfix.ini\n"
                f"❌ 您选择的是：{filename}\n\n"
...
def copy_full_code():
    ...
    for ini_key, var in tab["vars"].items():
        ...
        if ctrl and str(ctrl.cget("state")) != "disabled":
            ...
            if val != "": ui_data[ini_key.lower()] = (ini_key, val)
...
def restore_default():
    ...
    if "image" not in base_keys_lower: lines.append(f"Image={obj_id}")
...
tk.Button(frame_top, text="📂 绑定工程文件 (限 hotfix.ini)", command=choose_file, bg="#333333", fg="white", font=("", 10, "bold")).pack(side=tk.LEFT)
lbl_file = tk.Label(frame_top, text="当前目标: 未选择", bg="#1e1e1e", fg="red", font=("", 10, "bold"))
lbl_file.pack(side=tk.LEFT, padx=15)

tk.Button(frame_top, text="💾 测试部署 (Ctrl+S)", bg="darkred", fg="white", font=("", 10, "bold"), command=deploy).pack(side=tk.RIGHT)
