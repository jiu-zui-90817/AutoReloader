...
from PySide6.QtGui import QAction, QActionGroup, QKeySequence, QTextCursor, QPainter, QColor, QFont, QTextFormat
...
class MainWindow(QMainWindow):
    ...
    def open_settings(self):
        ...
        self.project.save_config()
        QMessageBox.information(self, "首选项", "已保存。新打开的调试窗口会使用新的辅助模式设置。")
    ...
    def save_all(self):
        ...
        for key in sorted(keys):
            ...
            backup_root = (self.project.project_dir / "backups") if self.project.project_dir else (path.parent / "backups")
            if not self.project.project_dir:
                result = self._save_with_backup_root(sid, body, path, False, peers, backup_root)
            else:
                result = self.project.save_section_text(
                    sid, body, target_path=Path(path), is_new=False, peer_ids=peers
                )
            if result.get("ok"):
                ok_n += 1
    ...
    def save_current(self):
        ...
        backup_root = (self.project.project_dir / "backups") if self.project.project_dir else (path.parent / "backups")
        # 临时确保 project_dir 有值供 save 内备份；无工程时用文件旁 backups
        if not self.project.project_dir:
            result = self._save_with_backup_root(section_id, body, path, is_new, peers, backup_root)
        else:
            result = self.project.save_section_text(
                section_id, body, target_path=Path(path), is_new=is_new, peer_ids=peers
            )
        if result.get("ok"):
            self._dirty = False
            # 写盘后同步内存为已保存内容
            ...
            QMessageBox.information(self, "保存成功", result.get("message", "已写入磁盘"))
            self.statusBar().showMessage(f"已保存到磁盘 {section_id} → {result.get('path')}", 8000)
        else:
